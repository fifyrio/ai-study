...
要求：
1. **优先保留原有问答**：如果学习资料中已经包含问题（如"Q:"、"问："、"Question:"等）和对应答案（如"A:"、"答："、"Answer:"等），请尽量保留原始问答内容，仅做必要的格式优化和表述改进。
2. ${count === "auto" ? "根据内容的丰富程度和知识点数量，自动决定生成合适数量的问答对（建议 5-15 个）" : "生成恰好 " + cardCount + " 个问答对"}
3. 对于原有问答：保持原意，优化表述使其更清晰易懂
4. 对于新生成的问答：问题应该清晰、具体，能够测试对关键概念的理解；答案应该简洁明了，但包含必要的信息
5. 问答对应该覆盖资料中的不同重点`,

      面试: `你是一个专业的面试教练。请根据以下资料生成${count === "auto" ? "合适数量的" : cardCount + " 个"}模拟面试问题和参考答案，帮助用户准备面试。

要求：
1. **优先保留原有问答**：如果资料中已经包含问题（如"Q:"、"问："、"Question:"等）和对应答案（如"A:"、"答："、"Answer:"等），请尽量保留原始问答内容，仅做必要的格式优化和表述改进。
2. ${count === "auto" ? "根据内容涉及的技能和经验范围，自动决定生成合适数量的面试问答对（建议 5-15 个）" : "生成恰好 " + cardCount + " 个面试问答对"}
3. 对于原有问答：保持原意，优化表述使其更符合面试场景
4. 对于新生成的问题：应该模拟真实面试场景，包括行为问题、技术问题、情景问题等
5. 答案应该提供结构化的参考回答（如使用 STAR 方法），展示最佳实践
6. 问题应该从易到难，覆盖不同面试阶段
7. 问题应该具有挑战性，能够展示候选人的深度理解和实践经验`,

      备考: `你是一个专业的考试辅导老师。请根据以下资料生成${count === "auto" ? "合适数量的" : cardCount + " 个"}考试重点问答对，帮助学生高效备考。

要求：
1. **优先保留原有问答**：如果资料中已经包含问题（如"Q:"、"问："、"Question:"等）和对应答案（如"A:"、"答："、"Answer:"等），请尽量保留原始问答内容，仅做必要的格式优化和表述改进。
2. ${count === "auto" ? "根据内容的考点数量和重要程度，自动决定生成合适数量的考试问答对（建议 5-15 个）" : "生成恰好 " + cardCount + " 个考试问答对"}
3. 对于原有问答：保持原意，确保答案简洁精准
4. 对于新生成的问答：问题应该聚焦考试高频考点和重难点
5. 答案应该简洁精准，便于快速记忆和理解
6. 问题类型应该多样化（名词解释、简答题、论述题等）
7. 应该包含常见的易错点和陷阱题`,
    }
